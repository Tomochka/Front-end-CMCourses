...
'using strict';//должен поправить????

class Game {
    constructor({ p1, p2 }) {
          
        this.player1 = p1;
        this.player2 = p2;//должен поправить пробел
      
        this.isPlayer1Win = undefined;
    }

    play() {
        var player1Win = Boolean(Math.round(Math.random()));//должен поставить пробел
        switch (player1Win) {
            case true:
...
class Championship {
    ...
    setSchedule() {
        ...
        for (var i = 0; i < this.gamers.length; i++) {
            for (var j = 0; j < this.gamers.length; j++) {
                if (i < j) this.currentGame.push(new Game({ p1: this.gamers[i], p2: this.gamers[j] }));
            }
        }
